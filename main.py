...
def _env_int(name, default=0):
    ...
    try:
        ...
        raise RuntimeError(f"متغیر محیطی {name} باید عدد صحیح باشه، مقدار فعلی: {val!r}")


TOKEN = os.environ.get("BOT_TOKEN", "").strip()
if not TOKEN:
    raise RuntimeError("متغیر محیطی BOT_TOKEN تنظیم نشده! توی Railway -> Variables اضافه‌ش کن.")

ADMIN_ID = _env_int("ADMIN_ID")
if not ADMIN_ID:
    raise RuntimeError("متغیر محیطی ADMIN_ID تنظیم نشده!")

GROUP_1 = os.environ.get("GROUP_1_LINK", "").strip()   # لینک عمومی گروه (مثلا https://t.me/xxxx)
GROUP_2 = os.environ.get("GROUP_2_LINK", "").strip()   # لینک عمومی کانال

GROUP_1_ID = _env_int("GROUP_1_ID")     # آیدی عددی گروه (منفی، مثلا -100xxxxxxxxxx)
CHANNEL_ID = _env_int("CHANNEL_ID")     # آیدی عددی کانال (منفی، مثلا -100xxxxxxxxxx)

# کانال‌هایی که کاربر باید عضوشون باشه، جدا شده با کاما، مثلا: @channel1,@channel2
REQUIRED_CHANNELS = [c.strip() for c in os.environ.get("REQUIRED_CHANNELS", "").split(",") if c.strip()]

# مسیر فایل دیتابیس. اگه روی Railway یه Volume وصل کردی، این متغیر رو روی مسیر
# مانت‌شده‌ی همون Volume بذار (مثلا /data/game.db) تا دیتا با هر دیپلوی پاک نشه.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_member:
        channels_text = "\n".join(f"{i+1}️⃣ {ch}" for i, ch in enumerate(REQUIRED_CHANNELS))
...
async def alliance_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    یک تابع واحد و اصلاح‌شده که هر سه حالت متن آزاد بیرون از ConversationHandler
    رو مدیریت می‌کنه: دلیل حمله اتمی، اسم اتحاد جدید، و درصد دلخواه حمله گروهی.
    (نسخه‌ی قبلی این تابع دو بار تعریف شده بود و نسخه‌ی دوم بی‌سروصدا نسخه‌ی اول
    رو خنثی می‌کرد؛ اینجا اصلاح شده.)
    """
    user_id = update.effective_user.id
